...
"type": "module",
"scripts": {
"start": "node --watch backend/src/index.js"
},

# .js: This is the default extension for the vast majority of JavaScript code today. In modern projects, developers use ES Modules (import/export syntax) within .js files by adding "type": "module" to their package.json file. This tells the environment (like Node.js or modern build tools) to treat all .js files in that package as modules.

# .mjs: If package.json ma => "type": "module" nalekhi ES6 modules like import/export use garna paryo vane .mjs use hunxa.

This extension (short for "module JavaScript") is used to explicitly inform the runtime that a specific file is an ES Module, regardless of the type setting in package.json. It was introduced during the transition to ES Modules in Node.js to avoid breaking backward compatibility with older CommonJS (require()) files. It is less common in general use and primarily seen in specific libraries or projects that need to mix both CommonJS and ES Module files in the same package.
